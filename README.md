...
* **Quad-State Logging:** Distinguish between Maintenance (solo), Playtime (recreational), Baby-Making (conception), and Clinical-Lab (medical baselines) with single-click action buttons.
...
* **Pre-Log Modifiers:** Track crucial biological variables like subjective volume (Dry/Low/Normal/High), heat/fever exposure (which impacts spermatogenesis), and supplement stacks (Zinc, Maca).
* **Interactive Charting:** Pan/zoom-enabled timeline generated via ApexCharts, featuring custom tooltips that display the clinical metadata, lab metrics, and supplement stacks for every recorded gap.
